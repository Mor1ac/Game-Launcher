...
cards.forEach(card => { card.dataset.text = card.querySelector("p").textContent.trim(); });
...
forSearch.addEventListener("input", function () {
  ...
  cards.forEach(card => {
    ...
    if (!value) { card.classList.remove("hide"); textEl.innerHTML = orig; return; }
    const idx = orig.toLowerCase().indexOf(value);
    if (idx === -1) {
      card.classList.add("hide");
    } else {
      card.classList.remove("hide");
      textEl.innerHTML = orig.slice(0,idx) + `<span class="highlight">${orig.slice(idx,idx+value.length)}</span>` + orig.slice(idx+value.length);
    }
  });
});

// ===== ЗАКРЫТИЕ ПО КЛИКУ СНАРУЖИ =====
[m1, m2, m3].forEach(p => p.addEventListener("click", e => e.stopPropagation()));
...
const gameData = {
  ...
  'search__card__5': { title:'Terraria', cover:'https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/105600/header.jpg?t=1731252354', badge:'Платная', tags:['Песочница','Выживание','Крафт'], desc:'Копайте, сражайтесь, исследуйте, стройте!', dev:'Re-Logic', date:'16.05.2011', price:'419 ₽', link:'/game-launcher/4/index2.html' }
};
...
function openGamePopup(cls) {
  const d = gameData[cls]; if (!d) return;
...
Object.keys(gameData).forEach(cls => {
  ...
  if (!card) return;
  card.addEventListener("click", e => { e.preventDefault(); e.stopPropagation(); searchCont.style.display = "none"; openGamePopup(cls); });
});
...
document.querySelector(".m3__clear").addEventListener("click", e => {
  e.stopPropagation();
  document.querySelector(".m3__list").innerHTML = `<div style="padding:30px 16px;text-align:center;color:rgba(255,255,255,0.3);font-size:13px;">Нет уведомлений</div>`;
});
...
document.querySelectorAll(".m2__chat-item").forEach(item => {
  item.addEventListener("click", e => {
    ...
    chatAvatar.innerHTML = item.dataset.avatarLetter + '<span class="m2__av-status m2__av-status--on"></span>';
...
document.querySelector(".action__button--save").addEventListener("click", (e) => {
  e.stopPropagation();
  launcherContainer.style.display = "none";
});
